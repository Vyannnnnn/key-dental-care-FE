        ...
        <thead className="bg-gray-50 dark:bg-gray-800">
          <tr>
            {headers.map((header, index) => (
              <th key={index} scope="col" className="py-3.5 px-4 text-sm font-semibold text-left rtl:text-right text-gray-500 dark:text-gray-400">
                {header.display}
              </th>
            ))}
            <th className="py-3.5 text-sm font-semibold text-center rtl:text-right text-gray-500 dark:text-gray-400">Aksi</th>
          </tr>
        </thead>
            ...
            return (
              <tr key={row.id}>
                {headers.map((header, colIndex) => (
                  <td key={colIndex} className="py-3.5 px-4 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    {row[header.field]}
                  </td>
                ))}
                <td>
                  <div className="w-full h-full flex justify-center gap-3">
                    ...
                    {/* Tombol Kedua */}
                    {actionButtonLabel2 && (
                      <button onClick={() => onActionButtonClick2(row)} className="hover:text-white border p-2 hover:bg-[#21695c] rounded-md min-w-[100px] flex items-center justify-center text-[#21695c]">
                        <span className="mr-2">{getIconComponent(iconType)}</span>
                        {actionButtonLabel2}
                      </button>
                    )}
